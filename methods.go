...
// SendMessageAlbum Sends messages grouped together into an album. Currently only audio, document, photo and video messages can be grouped into an album. Documents and audio files can be only groupув in an album with messages of the same type. Returns sent messages
// @param chatID Target chat
...
// @param _json The JSON-serialized string
func (client *Client) GetJsonValue(_json string) (JsonValue, error) {
	result, err := client.SendAndCatch(UpdateData{
		"@type": "getJsonValue",
		"json":  _json,
	})
